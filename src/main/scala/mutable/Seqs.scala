...
@State(Scope.Thread)
class Seqs:
  // O(1) no copying for head, tail and prepend, but O(n) for lookup by index and full-copying on append
  var list: List[Int] = uninitialized 
  
  // eC (more precisely O(log n)) for most of operations, plus minimal copying on tail or prepend
